...
ssh -o BatchMode=yes -o ConnectTimeout=10 -p $SSH_PORT "$RASPI_USER@$RASPI_IP" "mkdir -p $REMOTE_PATH"

# 2. Subir el archivo ZIP
Write-Host "Subiendo el paquete..."
scp -o BatchMode=yes -o ConnectTimeout=10 -P $SSH_PORT "mi-portafolio.zip" "${RASPI_USER}@${RASPI_IP}:${REMOTE_PATH}/mi-portafolio.zip"
...
ssh -o BatchMode=yes -o ConnectTimeout=10 -p $SSH_PORT "$RASPI_USER@$RASPI_IP" $deployCommand
